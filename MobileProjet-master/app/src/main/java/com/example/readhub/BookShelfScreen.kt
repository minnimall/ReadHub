    ...
    LaunchedEffect(lifecycleState){
        when (lifecycleState){
            ...
            Lifecycle.State.RESUMED -> {
                showAllBookShelf(bookItemsList, contextForToast)
            }
        }
    }
            ...
            Column(
                ...
                Column{
                        ...
                        Button(
                            onClick = {
                                if(textFieldName.trim().isEmpty()){
                                    ...
                                    bookItemsList.clear()
                                    createClient.SearchOnBookShelf(userId,textFieldName)
                    ...
                    )
                    {
                        ...
                        {
                            Text(
                                text = "ผลการค้นหา : ${textFieldName} ${bookItemsList.size} รายการ",
                                fontSize = 18.sp
                            )
                        }
                    }
                        ...
                        itemsIndexed(
                            ...
                            Card(
                                ...
                                Column (
                                    ...
                                    IconButton(
                                        onClick = {
                                            createClient.DeleteBookOnShelf(userId, item.book_id)
                                                .enqueue(object : Callback<Void> {
                                                    override fun onResponse(call: Call<Void>, response: Response<Void>) {
                                                        if (response.isSuccessful) {
                                                            // ลบหนังสือออกจาก bookItemsList เมื่อลบออกจากฐานข้อมูลสำเร็จ
                                                            bookItemsList.remove(item)
                                                            Toast.makeText(contextForToast, "Successfully Remove", Toast.LENGTH_LONG).show()
                                                        } else {
                                                            Toast.makeText(contextForToast, "Remove Failure", Toast.LENGTH_LONG).show()
                                                        }
                                                    }

                                                    override fun onFailure(call: Call<Void>, t: Throwable) {
                                                        Toast.makeText(contextForToast, "Error Failure" + t.message, Toast.LENGTH_LONG).show()
                                                    }
                                                })
                                        }
                                        ,
...
fun showAllBookShelf(bookItemsList: MutableList<Book>, context: Context){
    val createClient = UserApi.create()
    val userId = SharedPreferencesManager(context).userId
    createClient.MyBookShelf(userId)
        .enqueue(object : Callback<List<Book>> {
